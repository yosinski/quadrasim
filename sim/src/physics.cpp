...
void initPhysics()
{
	...
	//check for HW
	//gHardwareSimulation=false; //cancel hw sim anyway for now
	...
	if (!gCookingInitialized) {
		...
		if (!InitCooking(NULL, &gErrorStream)) 
			systemError("Unable to initialize the cooking library");
	}
	...
	//sceneDesc.gravity = NxVec3(0.0f, -9.81f*100, 0.0f);
	gScene = gPhysicsSDK->createScene(sceneDesc);
	if(gScene == NULL) 
		systemError("Error: Unable to create a PhysX scene, exiting the sample.\n\n");
...
NxActor* createCapsule(NxVec3& pos, float length/*=1*/, bool centered/*=true*/, float radius )
{
	NxActorDesc actorDesc;
	NxBodyDesc bodyDesc;
	//bodyDesc.angularDamping=50.0f; extreme val
